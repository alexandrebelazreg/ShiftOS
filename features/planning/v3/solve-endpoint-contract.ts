 ...
 *
 * One engine today. The field is kept rather than dropped because it is what
 * every stored result and every support log names its engine by; removing it
 * would make a schedule generated last month indistinguishable from one
 * generated by whatever comes next.
 */
export const PLANNING_V3_ENGINES = ["highs-fast"] as const
export type PlanningV3Engine = (typeof PLANNING_V3_ENGINES)[number]

/** The engine a request without an explicit choice gets. */
export const PLANNING_V3_DEFAULT_ENGINE: PlanningV3Engine = "highs-fast"
