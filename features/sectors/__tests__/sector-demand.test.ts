...
describe("configuration de demande secteur", () => {
  ...
  it("valide et persiste la coupure maximale du secteur", async () => { const valid = { ...complete(), shiftRules: { ...complete().shiftRules, splitShiftAllowed: true, maximumSplitDuration: 90 } }; expect(validateSectorDemand(valid, store)).toEqual([]); let value: string | null = null; const repository = createSectorRepository({ getItem: () => value, setItem: (_key, next) => { value = next } }); await repository.save([valid]); expect((await repository.list())[0].shiftRules.maximumSplitDuration).toBe(90); const invalid = { ...valid, shiftRules: { ...valid.shiftRules, maximumSplitDuration: 100 } }; expect(validateSectorDemand(invalid, store).some((issue) => issue.path === "shiftRules.maximumSplitDuration")).toBe(true) })
  it("migre explicitement les secteurs historiques Drive vers les jours obligatoires", async () => {
    ...
    expect((await repository.list())[0].workEveryNonFixedRestDay).toBe(true)
  })
  it("conserve explicitement false lors de la migration", async () => {
    let value: string | null = JSON.stringify([{ ...complete(), workEveryNonFixedRestDay: false }])
    const repository = createSectorRepository({ getItem: () => value, setItem: (_key, next) => { value = next } })
    expect((await repository.list())[0].workEveryNonFixedRestDay).toBe(false)
  })
  ...
  it("persiste l'appartenance explicite à Zone marché et ne l'invente pas à la migration", async () => {
    let value: string | null = JSON.stringify([{ ...complete(), marketZone: true }])
    const repository = createSectorRepository({ getItem: () => value, setItem: (_key, next) => { value = next } })
    expect((await repository.list())[0].marketZone).toBe(true)
    value = JSON.stringify([{ ...complete(), marketZone: undefined }])
    expect((await repository.list())[0].marketZone).toBe(false)
  })
  it("désactive une seule fois les anciennes colonnes de parts horaires", async () => {
