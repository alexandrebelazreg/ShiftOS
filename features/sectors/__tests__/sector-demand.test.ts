...
import { buildHourlyProfile, copyCoverageProfile, createEmptySector, createSectorRepository, validateSectorDemand } from "@/features/sectors"
...
function complete() { const base = createEmptySector("sector_1"); return { ...base, name: "Accueil", hours: base.hours.map((day) => day.day === "monday" || day.day === "tuesday" ? { ...day, closed: false } : day), coverage: { standardDay: "monday" as const, profiles: { monday: buildHourlyProfile("09:00", "18:00", 2), tuesday: buildHourlyProfile("09:00", "18:00", 2) } }, weeklyDistribution: { monday: 50, tuesday: 50, wednesday: 0, thursday: 0, friday: 0, saturday: 0, sunday: 0 }, competencies: [{ id: "skill_1", name: "Conseil", archived: false, order: 0 }] } }
